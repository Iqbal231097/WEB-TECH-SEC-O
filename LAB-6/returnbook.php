...
{
	$cn=makeconnection();
	$s="delete from  borrowbook  where bookname='" . $_POST["t2"] . "'";
	mysqli_query($cn,$s);
	mysqli_close($cn);
...
{
$cn=makeconnection();
$s="select * from  borrowbook where b_id='" .$_POST["t1"] ."'";
$result=mysqli_query($cn,$s);
$r=mysqli_num_rows($result);
...
<tr><td>&nbsp;</td><td ><input type="submit" value=" Return" name="sbmt" /></td></tr>
</table>
</form>
</div>
